    ...
    <div class="col-md-12">
        ...
        <div class="portlet light portlet-fit portlet-form bordered">
            ...
            <div class="portlet-body">
                <!-- BEGIN FORM-->
                <form action="{{url('admin/role/'.$role->id)}}" method="POST" id="editForm" class="form-horizontal">
                    {{ method_field('PUT') }}
                                    ...
                                    @foreach($permissions as $item)
                                        @if($role->hasPermissionTo($role->source, $item))
                                            <label class="col-md-6">
                                                <input type="checkbox" class="icheck" name="permissions[]" value="{{$item->name}}" checked> {{$item->name}} ({{$item->source}})
                                            </label>
                                        @else
                                            <label class="col-md-6">
                    ...
                    <div class="form-actions">
                        <div class="row">
                            <div class="col-md-offset-3 col-md-9">
                                ...
                                <a class="btn default" href="{{route('role.index')}}">Cancel</a>
                            </div>
                        </div>
                    </div>
                </form>
